...
class _NakesRevisiState extends State<NakesRevisi> {
      ...
      body: Stepper(
        ...
        steps: [
              ...
              child: Column(
                children: [
                  ...
                  if (nakes["hasil_dinkes"] == 2) ...[
                    Html(data: "<b>${nakes["catatan_dinkes"].toString()}</b>"),
                    const SizedBox(height: 5),
                    Html(
                      data:
                          "<span style='font-size: 11px;'>Apabila sampai dengan <b>${DateFormat('EEEE, d MMMM yyyy', 'id').format(DateTime.parse(nakes['jth_tempo_revisi'].toString()))}</b> Anda masih belum melengkapi berkas yang diminta, permohonan Anda akan batal secara otomatis.</span>",
                    ),
                  ],
                  const SizedBox(height: 5),
                  const Text(
          ...
          Step(
            ...
            subtitle: const Text(
              "DATA PERMOHONAN IZIN TENAGA KESEHATAN",
              style: TextStyle(fontSize: 10),
            ),
            content: Form(
              key: _permohonanKey,
                  ...
                  if (_tempatPraktek.text != "1") ...[
                    ...
                    DropdownSearch<FasyankesKategoriModels>(
                      asyncItems: (String filter) async {
                        ...
                        if (response.statusCode != 200) {
                          ...
                          List<FasyankesKategoriModels> kategoriFasyankesList =
                              [];

                          for (var element in kategoriFasyankes) {
                            kategoriFasyankesList
                                .add(FasyankesKategoriModels.fromJson(element));
                          }
                    ...
                    if (_idKategoriFasyankes.text != "") ...[
                      ...
                      DropdownSearch<FasyankesModels>(
                        ...
                        onChanged: (value) {
                          setState(() {
                            _idFasyankes.text = value?.id.toString() as String;
              ...
              content: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (varCekFasyankes != 0 && _tempatPraktek.text != '1')
                        ...
                        onPressed: () async {
                          if (_dokumenKey.currentState!.validate() &&
                            ...
                            var request = http.MultipartRequest(
                                'POST', Uri.parse("$baseUrl/api/nakes-revisi"));
                                ...
                                request.files.add(
                                  http.MultipartFile.fromBytes(
                                      ...
                                      File(scanIjasah![i].path)
                                          .readAsBytesSync(),
                                      filename:
                                          scanIjasah![i].path.split("/").last),
                                );
                              }
                            }

                            if (scanSertifikat != null) {
                              for (int i = 0; i < scanSertifikat!.length; i++) {
                            ...
                            if (scanStr != null) {
                              for (int i = 0; i < scanStr!.length; i++) {
                                request.files.add(http.MultipartFile.fromBytes(
                                    'dok_str[]',
                                    File(scanStr![i].path).readAsBytesSync(),
                                    filename:
                                        scanStr![i].path.split("/").last));
                              }
                            }
                              ...
                              for (int i = 0; i < scanKetSehat!.length; i++) {
                                request.files.add(http.MultipartFile.fromBytes(
                                    ...
                                    File(scanKetSehat![i].path)
                                        .readAsBytesSync(),
                                    filename:
                                        scanKetSehat![i].path.split("/").last));
                              }
                            }

                            if (scanSuperPatuh != null) {
                              for (int i = 0; i < scanSuperPatuh!.length; i++) {
                                request.files.add(http.MultipartFile.fromBytes(
                                    'sp_mematuhi[]',
                            ...
                            if (scanSppl != null) {
                              for (int i = 0; i < scanSppl!.length; i++) {
                                request.files.add(http.MultipartFile.fromBytes(
                                    'dok_sppl[]',
                                    File(scanSppl![i].path).readAsBytesSync(),
                                    filename:
                                        scanSppl![i].path.split("/").last));
                              }
                            }
                            ...
                            if (scanImb != null) {
                              for (int i = 0; i < scanImb!.length; i++) {
                                request.files.add(http.MultipartFile.fromBytes(
                                    'dok_imb[]',
                                    File(scanImb![i].path).readAsBytesSync(),
                                    filename:
                                        scanImb![i].path.split("/").last));
                              }
                            }
                            ...
                            if (scanKkpr != null) {
                              for (int i = 0; i < scanKkpr!.length; i++) {
                                request.files.add(http.MultipartFile.fromBytes(
                                    'dok_kkpr[]',
                                    File(scanKkpr![i].path).readAsBytesSync(),
                                    filename:
                                        scanKkpr![i].path.split("/").last));
                              }
                            }
                            ...
                            if (scanSlf != null) {
                              for (int i = 0; i < scanSlf!.length; i++) {
                                request.files.add(http.MultipartFile.fromBytes(
                                    'dok_slf[]',
                                    File(scanSlf![i].path).readAsBytesSync(),
                                    filename:
                                        scanSlf![i].path.split("/").last));
                              }
                            }
  ...
  Widget buildStr() => Column(
        children: [
          ...
          TextFormField(
            decoration: const InputDecoration(
              labelText: "MASA BERLAKU STR SAMPAI",
              labelStyle: TextStyle(fontSize: 13),
              suffixIcon: Icon(Icons.calendar_month),
            ),
